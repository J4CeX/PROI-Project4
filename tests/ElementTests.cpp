...
TEST(ElementTests, ElementCreator) {
...
TEST(ElementTests, ElementCreatorWithDefaultSetup) {
    Element element(10, 20, 30, 40);
    ASSERT_EQ(element.getX(), 10);
    ASSERT_EQ(element.getY(), 20);
    ASSERT_EQ(element.getWidth(), 30);
    ASSERT_EQ(element.getHeight(), 40);
    ASSERT_EQ(element.getFill(), "none");
    ASSERT_EQ(element.getStroke(), "none");
}